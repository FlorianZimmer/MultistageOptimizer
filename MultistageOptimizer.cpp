...
static string stripSpacesAndUnderscores(const string& value)
{
    string out;
...
static bool parseByteSizeString(const string& raw, unsigned long long& out)
{
    string value = stripSpacesAndUnderscores(raw);
    if (value.empty()) {
        return false;
    }
    ...
    string number_part = value.substr(0, pos);
...
static bool parseMaxRam(const json& value, unsigned long long& out)
{
    ...
    if (value.is_string()) {
        return parseByteSizeString(value.get<string>(), out);
    }
    return false;
}
